...
func _on_ground():
	var ball = get_node("CanvasLayer1/Player_Layer/ball")
	#print("Ist es auf dem Boden?")
	print(ball.get_colliding_bodies())
	return (ball.get_colliding_bodies().size() > 0)

func _process(delta):
	var ball_velocity = get_node("CanvasLayer1/Player_Layer/ball").get_linear_velocity()
	var ball_vel = Vector2(0,0)
	if (Input.is_action_pressed("move_right")):
		ball_vel.x += ball_speed * delta
		get_node("CanvasLayer1/Player_Layer/ball").set_linear_velocity(ball_velocity+ball_vel)
